...
const LeftChatHistory: React.FC<LeftChatHistoryProps> = ({ history, onMessageClick }) => {
  ...
  return (
    <div className={`font-sans border border-gray-300 rounded-lg max-w-[300px] h-screen flex flex-col transition-all duration-300 ease-in-out bg-gray-100 dark:bg-gray-800 ${
      isCollapsed ? 'w-10' : 'max-w-[300px]'
    }`}>
      <div className="flex-shrink-0">
        <div className="flex items-center gap-2 justify-between p-4 border-b border-gray-300 bg-inherit">
          <span 
          ...
          <span 
            ...
            className={`cursor-pointer transition-opacity duration-200 hover:opacity-70 ${isCollapsed ? 'hidden' : ''}`}
          >
            <Edit size={24} />
          </span>
        </div>
          ...
          {Object.entries(groupedHistory).map(([dateGroup, chats]) => (
            <div key={dateGroup} className="mb-4">
              ...
              {chats.map((chat) => (
                <div
                  key={chat.id}
                  className="px-4 py-3 cursor-pointer border-b border-gray-300 transition-colors duration-200 hover:bg-gray-200 dark:hover:bg-gray-700/50 last:border-b-0"
      ...
      <div className={`h-12 px-4 py-2 flex flex-row gap-2 items-center ${isCollapsed ? 'hidden' : ''}`}>
        <span>
          <Orbit size={24} />
        </span>
